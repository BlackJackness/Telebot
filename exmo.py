...
DEFAULT_FROM = datetime.utcfromtimestamp(int(time.time()) - SEC_PER_DAY).strftime('%Y-%m-%d %H:%M:%S')
DEFAULT_TO = datetime.utcfromtimestamp(int(time.time())).strftime('%Y-%m-%d %H:%M:%S')
...
class ExmoExchange:
    ...
    def print_graph(self, sell_coin, buy_coin='USD', date_from_format=DEFAULT_FROM, date_to_format=DEFAULT_TO,
                    format='%Y-%m-%d %H:%M:%S'):
        ...
        for item in r.json()['candles']:
            ...
            low.append(item['l'])
            close.append(item['c'])
            xdate.append(datetime.fromtimestamp(int(item['t']) // 1000))
        fig, ax = plt.subplots()
        ax.set_xlabel('Date')
    ...
    def do_analytics(self, sell_coin, buy_coin='USD', date_from_format=DEFAULT_FROM, date_to_format=DEFAULT_TO,
                     format='%Y-%m-%d %H:%M:%S'):
        date_from = int(time.mktime(datetime.strptime(date_from_format, format).timetuple()))
        date_to = int(time.mktime(datetime.strptime(date_to_format, format).timetuple()))
        r = requests.get(
        ...
        open_cost = float(r.json()['candles'][0]['o'])
        close_cost = float(r.json()['candles'][-1]['c'])
