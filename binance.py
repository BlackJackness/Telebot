...
class BinanceExchange:
    ...
    def print_graph(self, sell_coin, buy_coin='USDT', date_from_format=DEFAULT_FROM, date_to_format=DEFAULT_TO,
                    format='%Y-%m-%d %H:%M:%S'):
    ...
    def last_trades(self, sell_coin, buy_coin='USDT', limit=20):
        ...
        ind = 1
        for item in r.json():
            date = datetime.utcfromtimestamp(int(item['T']) // 1000).strftime('%Y-%m-%d %H:%M:%S')
    ...
    def do_analytics(self, sell_coin, buy_coin='USDT', date_from_format=DEFAULT_FROM, date_to_format=DEFAULT_TO,
                     format='%Y-%m-%d %H:%M:%S'):
        date_from = int(time.mktime(datetime.strptime(date_from_format, format).timetuple()))
        date_to = int(time.mktime(datetime.strptime(date_to_format, format).timetuple()))
        r = requests.get(
            f'https://www.binance.com/api/v1/klines?symbol={sell_coin}{buy_coin}&interval=1m&endTime={date_to * 1000}')
        open_cost = float(r.json()[0][1])
