...
def test_print_graph():
    for coin in coins:
        ...
        assert open(bin_res[11:])
        assert open(exmo_res[11:])
...
def test_last_trades():
    ...
    for coin in coins:
        ...
        assert re.match(
            'Последние сделки в паре ' + coin + '/USDT\n\t[0-9]+\. [0-9]{4}-[0-9]{2}-[0-9]{2} [0-9]{2}:[0-9]{2}:[0-9]{2} (buy|sell) [0-9.]+ ' + coin + ' for [0-9.]+ USDT were price was [0-9.]+',
            bin_res)
        assert re.match(
            'Последние сделки в паре ' + coin + '/USD\n\t[0-9]+\. [0-9]{4}-[0-9]{2}-[0-9]{2} [0-9]{2}:[0-9]{2}:[0-9]{2} (buy|sell) [0-9.]+ ' + coin + ' for [0-9.]+ USD were price was [0-9.]+',
            exmo_res)


real_changes = {'BTC': ['6455.89 USDT', '6455.02 USDT', '-0.87 USDT', '6634.43363601 USD', '6463.79999999 USD',
                        '-170.63363602 USD'],
                'ETH': ['209.75 USDT', '208.68 USDT', '-1.07 USDT', '223.60116459 USD', '209.5828455 USD',
                        '-14.01831909 USD'],
                'ETC': ['9.4607 USDT', '9.3613 USDT', '-0.0994 USDT', '11.12185252 USD', '9.3507 USD',
                        '-1.77115252 USD'],
                'ADA': ['0.07561 USDT', '0.07677 USDT', '0.00116 USDT', '0.0829955 USD', '0.0769382 USD',
                        '-0.0060573 USD']}
...
def test_do_analytics():
    for coin in coins:
        ...
        bin_res = resolve_method('bin', 'do_analytics', f'{coin}_USDT', dates)
        exmo_res = resolve_method('exmo', 'do_analytics', f'{coin}_USD', dates)
        changes = []
